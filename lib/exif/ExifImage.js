...
require('./Buffer'); // Extend Buffer.prototype
...
function ExifImage (options, callback) {
  ...
  var ops = this.options = Object.assign({}, options);
...
ExifImage.prototype.extractExifData = function (data, start, length) {
  ...
  if (typeof exifData.exif[ExifImage.TAGS.exif[0x927C]] != "undefined") {
    ...
    if (exifData.exif[ExifImage.TAGS.exif[0x927C]].getString(0, 7) === "OLYMP\x00\x01" || exifData.exif[ExifImage.TAGS.exif[0x927C]].getString(0, 7) === "OLYMP\x00\x02") {
      // type="olympus"
      // Olympus maker notes is broken and need more example images to be fixed
    ...
    } else if (exifData.image.Make === "Canon") {
      type="canon";
    }

    debug("Makernote IFD ifdOffset=", ifdOffset, "type=", type);
    debug('Maker note offsets: ', this.offsets.makernoteOffset, tiffOffset);
...
ExifImage.prototype.extractExifEntry = function (data, entryOffset, tiffOffset, isBigEndian, tags) {
  ...
  // Tag contains child tags, corisponding tag entry provides object with tag
  // value and format() function to help extract it
  } else if (tags && tags[entry.tagId] && typeof tags[entry.tagId] == "object") {
    entry.tagName = tags[entry.tagId].tag;
    if (!entry.tagName) {
      return false;
    }
    entry.valueOffset = (entry.components == 1) ? entryOffset + 8 : data.getLong(entryOffset + 8, isBigEndian) + tiffOffset;
    entry.value = tags[entry.tagId].format(entry.valueOffset, data);
    return entry;
  ...
  switch (entry.format) {
    ...
    case 0x0001: // unsigned byte, 1 byte per component
      ...
      for (var i = 0, pos = entry.valueOffset; i < entry.components && pos < data.length; i++, pos += 1)
        entry.value.push(data.getByte(pos));
      break;
    ...
    case 0x0003: // unsigned short, 2 byte per component
      ...
      for (var i = 0, pos = entry.valueOffset; i < entry.components && pos < data.length - 1; i++, pos += 2)
        entry.value.push(data.getShort(pos, isBigEndian));
      break;

    case 0x0004: // unsigned long, 4 byte per component
      entry.valueOffset = (entry.components == 1) ? entryOffset + 8 : data.getLong(entryOffset + 8, isBigEndian) + tiffOffset;
      for (var i = 0, pos = entry.valueOffset; i < entry.components && pos < data.length - 3; i++, pos += 4)
        entry.value.push(data.getLong(pos, isBigEndian));
      break;
    ...
    case 0x0005: // unsigned rational, 8 byte per component (4 byte numerator and 4 byte denominator)
      ...
      for (var i = 0, pos = entry.valueOffset; i < entry.components && pos < data.length - 7; i++, pos += 8)
        entry.value.push(data.getLong(pos, isBigEndian) / data.getLong(pos + 4, isBigEndian));
      break;

    case 0x0006: // signed byte, 1 byte per component
      entry.valueOffset = (entry.components <= 4) ? entryOffset + 8 : data.getLong(entryOffset + 8, isBigEndian) + tiffOffset;
      for (var i = 0, pos = entry.valueOffset; i < entry.components && pos < data.length; i++, pos += 1)
        entry.value.push(data.getSignedByte(pos));
      break;
    ...
    case 0x0008: // signed short, 2 byte per component
      entry.valueOffset = (entry.components <= 2) ? entryOffset + 8 : data.getLong(entryOffset + 8, isBigEndian) + tiffOffset;
      for (var i = 0, pos = entry.valueOffset; i < entry.components && pos < data.length - 1; i++, pos += 2)
        entry.value.push(data.getSignedShort(pos, isBigEndian));
      break;

    case 0x0009: // signed long, 4 byte per component
      entry.valueOffset = (entry.components == 1) ? entryOffset + 8 : data.getLong(entryOffset + 8, isBigEndian) + tiffOffset;
      for (var i = 0, pos = entry.valueOffset; i < entry.components && pos < data.length - 3; i++, pos += 4)
        entry.value.push(data.getSignedLong(pos, isBigEndian));
      break;

    case 0x000A: // signed rational, 8 byte per component (4 byte numerator and 4 byte denominator)
      entry.valueOffset = data.getLong(entryOffset + 8, isBigEndian) + tiffOffset;
      for (var i = 0, pos = entry.valueOffset; i < entry.components && pos < data.length - 7; i++, pos += 8)
        entry.value.push(data.getSignedLong(pos, isBigEndian) / data.getSignedLong(pos + 4, isBigEndian));
      break;
  ...
  if (entry.tagName === "MakerNote") {
    debug('MakerNoteOffset', entry.valueOffset, entry.components);
